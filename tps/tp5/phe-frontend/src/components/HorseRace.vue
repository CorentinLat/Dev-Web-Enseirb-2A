...
const emit = defineEmits(['update:raceState', 'supprimerParis']);
const props = defineProps(['chevaux']);

const endOfRace = ref(false);
const winningHorse = ref(null);
    ...
    interval.value = setInterval(() => {
        props.chevaux.forEach(horse => {
            ...
            if (horse.position > 560) {
                ...
                winningHorse.value = horse.value;
            }
        });
        ...
        if (endOfRace.value) {
            ...
            emit('update:raceState', {
                etat: 'Terminée',
                chevalGagnant: winningHorse.value,
            });
        }
    }, 50);
};
